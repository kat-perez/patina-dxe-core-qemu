//! QEMU Q35 SMBIOS Test Component
//!
...
#[derive(Default)]
pub struct Q35SmbiosTest;

#[component]
impl Q35SmbiosTest {
    /// Creates a new instance of the QEMU Q35 SMBIOS Test component.
    pub fn new() -> Self {
        Self
    }

    /// Entry point for the SMBIOS Test component.
    ///
    /// Locates the SMBIOS protocol and exercises the C FFI functions to verify
    /// the protocol boundary works correctly.
    pub fn entry_point(self, storage: &mut Storage) -> patina::error::Result<()> {
        log::debug!("SMBIOS Test Entry Point - Testing C Protocol FFI Layer");

        Self::test_c_protocol_layer(storage.boot_services())?;

        log::debug!("SMBIOS Test complete");
        Ok(())
    }

    /// Test the C Protocol FFI layer by calling the protocol functions directly
    fn test_c_protocol_layer(boot_services: &StandardBootServices) -> patina::error::Result<()> {
        ...
        if status != efi::Status::SUCCESS {
            log::trace!("    [FAIL] Protocol Add failed: {:?}", status);
        } else {
            ...
            if status != efi::Status::SUCCESS {
                log::trace!("    [FAIL] Protocol UpdateString failed: {:?}", status);
            } else {
            ...
            if status != efi::Status::SUCCESS {
                log::trace!("    [FAIL] Protocol Remove failed: {:?}", status);
            } else {
            ...
            if status == efi::Status::SUCCESS {
                log::trace!("    [FAIL] UpdateString after removal should have failed");
            } else {
            ...
            if status != efi::Status::SUCCESS {
                log::trace!("    [FAIL] Protocol GetNext (first) failed: {:?}", status);
            } else {
                ...
                if status != efi::Status::SUCCESS {
                    log::trace!("    [FAIL] Protocol GetNext (second) failed: {:?}", status);
                } else {
                    // Copy fields from packed struct to avoid unaligned reference
                    let (rec_type, rec_handle, rec_length) = unsafe {
                        let header = &*record_ptr;
